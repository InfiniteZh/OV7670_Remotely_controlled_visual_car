...
void jint_process_mcu(jpeg_compress_info *cinfo) 
{
  ...
  for (cn=0; cn<cinfo->num_comp; cn++) 
  {
	...
	temp_dct1[i] = val[i];
       ...
       */
      for (i=0; i<DCTSIZE2; i++) {
        val[i] = (DCTVAL)((float)val[i]/(float)quant_table->quantval[i]+0.5f);
      }
      #ifdef DEBUG
                      ...
                      float quality) 
{
  ...
  for (i=0; i<DCTSIZE2; i++) {
    qtblptr[i] = (INT16)(base_tbl[i] * quality + 0.5f);
  }
...
jint_std_quant_tables( jpeg_compress_info *cinfo ) {
  ...
  static const UINT16 std_chrominance_quant_tbl[DCTSIZE2] = {
