...
// Description	: SDRAM×´Ì¬¿ØÖÆÄ£¿é
//							SDRAM³õÊ¼»¯ÒÔ¼°¶¨Ê±Ë¢ÐÂ¡¢¶ÁÐ´¿ØÖÆ
//				
// Revision		: V1.0
// Additional Comments	:  
// 
...
wire sdram_busy;		// SDRAM忙标志，高表示SDRAM处于工作
...
//------------------------------------------------------------------------------
//ÉÏµçºó200us¼ÆÊ±,¼ÆÊ±Ê±¼äµ½,Ôòdone_200us=1
//------------------------------------------------------------------------------
reg[14:0] cnt_200us; 
always @ (posedge clk or negedge rst_n) 
	if(!rst_n) cnt_200us <= 15'd0;
	else if(cnt_200us < 15'd20_000) cnt_200us <= cnt_200us+1'b1;	//¼ÆÊý
...
//------------------------------------------------------------------------------
reg[3:0] init_state_r;	// SDRAM³õÊ¼»¯×´Ì¬

always @ (posedge clk or negedge rst_n)
	if(!rst_n) init_state_r <= `I_NOP;
	else 
		case (init_state_r)
				`I_NOP: 	init_state_r <= done_200us ? `I_PRE:`I_NOP;		//ÉÏµç¸´Î»ºó200us½áÊøÔò½øÈëÏÂÒ»×´Ì¬
				`I_PRE: 	init_state_r <= `I_TRP;		//预充电状态
				`I_TRP: 	init_state_r <= (`end_trp) ? `I_AR1:`I_TRP;			//Ô¤³äµçµÈ´ýTRP_CLK¸öÊ±ÖÓÖÜÆÚ
				`I_AR1: 	init_state_r <= `I_TRF1;	//第1次自刷新
				`I_TRF1:	init_state_r <= (`end_trfc) ? `I_AR2:`I_TRF1;			//µÈ´ýµÚ1´Î×ÔË¢ÐÂ½áÊø,TRFC_CLK¸öÊ±ÖÓÖÜÆÚ
				`I_AR2: 	init_state_r <= `I_TRF2; 	//第2次自刷新	
				`I_TRF2:	init_state_r <= (`end_trfc) ? `I_MRS:`I_TRF2; 		//µÈ´ýµÚ2´Î×ÔË¢ÐÂ½áÊø,TRFC_CLK¸öÊ±ÖÓÖÜÆÚ
				`I_MRS:		init_state_r <= `I_TMRD;////模式寄存器设置（MRS）	
				`I_TMRD:	init_state_r <= (`end_tmrd) ? `I_DONE:`I_TMRD;		//µÈ´ýÄ£Ê½¼Ä´æÆ÷ÉèÖÃÍê³É,TMRD_CLK¸öÊ±ÖÓÖÜÆÚ
